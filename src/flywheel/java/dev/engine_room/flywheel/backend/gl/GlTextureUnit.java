...
public enum GlTextureUnit {
	...
	public static GlTextureUnit getActive() {
		return fromGlEnum(activeTextureUnit());
	}

	/**
	 * 26.1 made GlStateManager's active-unit field private and dropped the getter, so ask the driver.
	 * Only called when Flywheel enters and leaves its own GL island, not per draw.
	 */
