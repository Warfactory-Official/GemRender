...
public final class FireComponent implements EntityComponent {
	// 26.1 dropped LightTexture.FULL_BLOCK; LightCoordsUtil kept the same packing.
	private static final int FULL_BLOCK = LightCoordsUtil.pack(15, 0);
	...
	private static final RendererReloadCache<SpriteId, Model> FIRE_MODELS = new RendererReloadCache<>(texture -> {
		// 26.1 replaced resources.model.Material with sprite.SpriteId, which is a pure id -- resolving
		// it to a sprite now goes through the atlas manager.
		return new SingleMeshModel(new FireMesh(Minecraft.getInstance()
				.getAtlasManager()
				.get(texture)), FIRE_MATERIAL);
	});
