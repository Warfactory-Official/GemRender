...
import static com.wf.gemrender.vendor.mcgltf.animation.GltfAnimationCreator.*;
...
public final class NodeTable {
    ...
    public boolean isPosable(int slot) {
        return slot >= 0 && slot < nodeCount && fixedLocal[slot] == null;
    }

    public float restTranslation(int slot, int axis) {
        return restState[slot * TRS_STRIDE + TRANSLATION + axis];
    }

    public void restRotation(int slot, Quaternionf out) {
        int base = slot * TRS_STRIDE + ROTATION;
        out.set(restState[base], restState[base + 1], restState[base + 2], restState[base + 3]);
    }

    public void setTranslation(float[] scratch, int slot, float x, float y, float z) {
        int base = slot * TRS_STRIDE + TRANSLATION;
        scratch[base] = x;
        scratch[base + 1] = y;
        scratch[base + 2] = z;
    }

    public void setRotation(float[] scratch, int slot, float x, float y, float z, float w) {
        int base = slot * TRS_STRIDE + ROTATION;
        scratch[base] = x;
        scratch[base + 1] = y;
        scratch[base + 2] = z;
    ...
    public void setRotation(float[] scratch, int slot, Quaternionfc rotation) {
        setRotation(scratch, slot, rotation.x(), rotation.y(), rotation.z(), rotation.w());
    }

    public void setScale(float[] scratch, int slot, float x, float y, float z) {
        int base = slot * TRS_STRIDE + SCALE;
        scratch[base] = x;
        scratch[base + 1] = y;
        scratch[base + 2] = z;
    }
