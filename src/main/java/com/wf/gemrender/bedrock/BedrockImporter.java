...
public final class BedrockImporter {
    ...
    public static GemRenderGltfModel load(ResourceLocation location, @Nullable ResourceLocation textureOverride)
            throws IOException {
        return load(location, location, textureOverride == null ? List.of() : List.of(textureOverride));
    }
    ...
    public static GemRenderGltfModel load(ResourceLocation location, ResourceLocation id,
        ...
        ResourceLocation textureOverride = skins.isEmpty() ? null : skins.get(0);
        ...
        ResourceLocation base = texture(geometry, location, textureOverride);
        ModelAtlas atlas = skins.size() < 2 || base == null ? null
                : ModelAtlas.stitch(atlasId(id), List.of(MaterialMaps.plain(base)), swaps(base, skins));

        GltfMaterial material;
        SpriteUv uv;
        if (atlas == null) {
            material = material(geometry, location, textureOverride, ownedTextures);
            uv = SpriteUv.IDENTITY;
        } else {
            ownedTextures.add(atlas.texture());
            material = new GltfMaterial(atlas.texture(), alphaMode(geometry.alpha()), 0.1f, !geometry.cull());
            uv = atlas.uv(MaterialMaps.plain(base));
        }
        ...
        for (Map.Entry<Integer, List<Part>> group : bySlot(skeleton).entrySet()) {
            ...
            MeshGeometry piece = MeshGeometry.of(builder.positions(), builder.normals(),
                    builder.texCoords(), builder.indices(), skinning, uv, 0);
        ...
        return new GemRenderGltfModel(model, layout, bounds.build(), GltfMorphLayout.NONE, animations,
                atlas == null ? null : atlas.texture(), ownedTextures,
                atlas == null ? List.of(VariantUv.NONE) : atlas.variants());
    }

    public static GemRenderPartsModel loadParts(ResourceLocation location) throws IOException {
        return loadParts(location, null);
    }

    public static GemRenderPartsModel loadParts(ResourceLocation location,
                                                @Nullable ResourceLocation textureOverride) throws IOException {
        ...
        List<ResourceLocation> ownedTextures = new ArrayList<>();
        GltfMaterial material = material(geometry, location, textureOverride, ownedTextures);
        Material colour = material.toFlywheel();
        Material depthOnly = GltfMaterial.depthPassFor(colour);
        ...
        PartMerge.Pixels sheet = sheet(geometry, location, textureOverride);
    ...
    private static PartMerge.Pixels sheet(BedrockGeometry geometry, ResourceLocation location,
        ...
        ResourceLocation source = texture(geometry, location, textureOverride);
        if (source == null) {
            return null;
        }
        ...
        try (image) {
            ...
            for (int y = 0; y < height; y++) {
                for (int x = 0; x < width; x++) {
                    argb[y * width + x] = Pixels.get(image, x, y);
                }
            }
    ...
    private static GltfMaterial material(BedrockGeometry geometry, ResourceLocation location,
                                         @Nullable ResourceLocation textureOverride, List<ResourceLocation> ownedTextures) {
        ResourceLocation texture = texture(geometry, location, textureOverride);
    ...
    private static List<Map<ResourceLocation, ResourceLocation>> swaps(ResourceLocation base,
                                                                       List<ResourceLocation> skins) {
